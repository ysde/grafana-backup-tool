...
def main(config_path):
    ...
    grafana_default_user_password = config.get("grafana", {}).get(
    ...
    backup_file_format = config.get("general", {}).get(
        "backup_file_format", "%Y%m%d%H%M"
    )
    uid_dashboard_slug_suffix = config.get("general", {}).get(
    ...
    azure_storage_connection_string = config.get("azure", {}).get(
        "connection_string", ""
    )
    # Cloud storage settings - GCP
    gcp_config = config.get("gcp", {})
    gcs_bucket_name = gcp_config.get("gcs_bucket_name", "")
    google_application_credentials = gcp_config.get(
        "google_application_credentials", ""
    )

    influxdb_measurement = config.get("influxdb", {}).get(
    ...
    DEFAULT_USER_PASSWORD = os.getenv(
        "DEFAULT_USER_PASSWORD", grafana_default_user_password
    )
    ...
    if isinstance(API_HEALTH_CHECK, str):
        API_HEALTH_CHECK = json.loads(
            API_HEALTH_CHECK.lower()
        )  # convert environment variable string to bool

    API_AUTH_CHECK = os.getenv("API_AUTH_CHECK", api_auth_check)
    if isinstance(API_AUTH_CHECK, str):
        API_AUTH_CHECK = json.loads(
            API_AUTH_CHECK.lower()
        )  # convert environment variable string to bool
    ...
    UID_DASHBOARD_SLUG_SUFFIX = os.getenv(
        "UID_DASHBOARD_SLUG_SUFFIX", uid_dashboard_slug_suffix
    )
    ...
    if isinstance(PRETTY_PRINT, str):
        PRETTY_PRINT = json.loads(
            PRETTY_PRINT.lower()
        )  # convert environment variable string to bool

    EXTRA_HEADERS = dict(
        h.split(":")
        for h in os.getenv("GRAFANA_HEADERS", "").split(",")
    ...
    if GRAFANA_BASIC_AUTH:
        ...
        HTTP_GET_HEADERS_BASIC_AUTH.update(
            {"Authorization": "Basic {0}".format(GRAFANA_BASIC_AUTH)}
        )
        HTTP_POST_HEADERS_BASIC_AUTH = HTTP_POST_HEADERS.copy()
        HTTP_POST_HEADERS_BASIC_AUTH.update(
            {"Authorization": "Basic {0}".format(GRAFANA_BASIC_AUTH)}
        )
    else:
